...
#
def use_lock(db_Lock, operation, nprocs=0):
    """ use_lock

        db_lock   = lock to be used
        operation = True => Lock
                  = False => Release
    ...
    if db_lock is None:
        logging.debug('use_lock: db_lock is [None].')
        return use_dblock_return

    logging.debug('use_lock: db_lock.semlock:[%s].', db_lock._semlock)
